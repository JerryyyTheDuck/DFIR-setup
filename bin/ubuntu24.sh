...
function network(){
    echo -e "${YELLOW}[*] Installing Wireshark${NORMAL}"
    sudo apt install wireshark -y > /dev/null 2>&1
...
function reverse(){
    ...
    echo -e "${YELLOW}[*] Installing dnSpy${NORMAL}"
    wget -q https://github.com/horsicq/DIE-engine/releases/download/3.10/die_3.10_Ubuntu_24.04_amd64.deb -P /home/$SUDO_USER/tools/die
    chmod +x /home/$SUDO_USER/tools/die/die_3.10_Ubuntu_24.04_amd64.deb
    sudo apt-get install -y /home/$SUDO_USER/tools/die/die_3.10_Ubuntu_24.04_amd64.deb
    writeToLog $? "DIE - Install successfully"

    echo -e "${YELLOW}[*] Installing apktool${NORMAL}"
    wget -q https://github.com/iBotPeaches/Apktool/releases/download/v2.10.0/apktool_2.10.0.jar -P /home/$SUDO_USER/tools/apktool
    chmod +x /home/$SUDO_USER/tools/apktool/apktool_2.10.0.jar
    writeToLog $? "apktool - Install successfully"
}
...
function stego(){
    ...
    sudo apt-get install -y  /home/$SUDO_USER/tools/stegseek/stegseek_0.6-1.deb
...
function misc(){
    ...
    sudo apt install apt-transport-https ca-certificates curl software-properties-common > /dev/null 2>&1
    curl -fsSL https://download.docker.com/linux/ubuntu/gpg > /dev/null 2>&1 | sudo apt-key add - > /dev/null 2>&1
    ...
    writeToLog $? "docker - Install successfully"    
    ...
    echo -e "${YELLOW}[*] Installing pwndbg${NORMAL}"
    git clone https://github.com/pwndbg/pwndbg $TMP_DIR/pwndbg > /dev/null 2>&1
    cd $TMP_DIR/pwndbg > /dev/null 2>&1 && chmod +x setup.sh > /dev/null 2>&1 && bash setup.sh > /dev/null 2>&1
    writeToLog $? "pwndbg - Install successfully"
}   
